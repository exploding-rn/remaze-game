const {generate,slideCoverage,canVisitBeacons,traceMove,specialCoverage,decorateLevel}=require('./game.js');
...
for(let stage=1;stage<=40;stage++){
 ...
 if(JSON.stringify(a)!==JSON.stringify(b)||!specialCoverage(a.grid,a.start,a,a.beacons||[]))throw Error(`Special stage ${stage} changed or blocked coverage.`);
}
for(let level=1;level<=120;level++)for(const brain of [false,true]){
 ...
 if(!specialCoverage(maze.grid,maze.start,maze,maze.beacons||[]))throw Error(`Classic/Brain special level ${level} blocks coverage.`);
}
